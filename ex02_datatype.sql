...
1. 숫자형
    ...
    a. number
        ...
        - number : 정수 or 실수
        - number(proecision) : 정수만 저장
        - number(proecision, scale ): 전체 자릿수 > 정수 /실수 저장
...
2. 문자형 > varchar2 사용
    ...
    b. nchar
        - n: natioanl > 오라클 인코딩과 상관없이 해당 컬럼을 UTF16으로 동작
...
*/
-- 헝가리안 표기법 > 저료형을 접두어로 사용
create table tblType (
    --num number 
    --num number(3)
    --num number(4, 2) --최대자리수 -999 ~ 999, 기본값 38
    --txt1 char(10), -- 오라클이 어떤 인코딩? UTF-8
    --txt2 varchar2(10)
    txt nchar(10)
);
...
insert into tblType (num) values (99.9);
insert into tblType (num) values (-99.9);
