...
/*
    ...
    2. null이 많이 발생하는 상태의 테이블 > 공간 낭비 +  SQL 작업 불편
...
--ORA-02291: 무결성 제약조건(HR.SYS_C008489)이 위배되었습니다- 부모 키가 없습니다
...
/*
    ...
    1. 단순 조인, cross join, 카티션 곱, 테카르드 곱
    - 모든 조인의 기본 동작
    - A 테이블 X B 테이블
    ...
    select 컬럼 리스트  from 테이블A cross join  테이블B;
    
    select 컬럼 리스트  from 테이블A inner join  테이블B on 테이블A.PK = 테이블B.FK;
...
select
    m.name, (select tel from tblMember where name = m.name), count(*)
from tblMember m
    inner join tblRent r
        on m.seq = r.member
            group by m.name;  
...
create table tblSelf(
    ...
    name varchar2(30) not null,                 --직원명
    department varchar(30) not null,            --부서명
...
create table tblProject (
    seq number primary key,         --프로젝트번호(PK)
    project varchar2(100) not null, --프로젝트명
    staff_seq number null references tblStaff(seq)       --담당 직원번호(FK)
); -- 자식테이블
...
insert into tblProject (seq, project, staff_seq) values (6, '재고관리', null);
