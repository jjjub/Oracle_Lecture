/*
    ...
    - 논리 > 관계형 데이터베이스 기본 설정 > 속성(자료형, 길이) + 도메인 + 키....
    ...
    - 릴레이션 = 엔티티 = 레코드 = 객체 > 집합(=테이블, 클래스)
        a. 학생 정보 관리
            - 정보 수집 : 아이디, 학생명, 나이, 주소, 연락처 등..
            - 학생(아이디, 학생명, 나이, 주소, 연락처) > 엔티티
            
        b. 강의실 정보 관리
            - 정보 수집 : 강의실명, 크기, 수용 인원, 용도 등...
            - 강의실(강의실명, 크기, 수용 인원, 용도) > 엔티티
    ...
    ERD > Entity, Attributem Relationship 등 표현하는 방법
    - 피터첸 or IE
    ...
    이상 현상
    1. 삽입 이상
        - 특정 테이블에 데이터를 삽입할 때 원하는 않는 데이터까지 같이 넣어야 하는 상황
        
    2. 갱신 이상
        - 동일한 데이터가 2개 이상의 테이블에 동시에 저장되는 현상
        
    3. 삭제 이상
        - 특정 테이블에서 데이터를 삭제할 떄 원하지 않는 데이터까지 같이 삭제해야 하는 현상
