...
select name as 국가명, capital as 수도명, population as 인구수 from tblCountry where name = '대한민국';
...
select first_name || ' '|| last_name, email as 이메일, phone_number as 연락처, salary as 급여 from employees;
...
select name, capital, area from tblCountry where area <= 100;
...
select first_name || ' '|| last_name as 이름, phone_number as 연락처, email as  이메일, job_id, department_id from employees where department_id in (60, 80, 100);
...
select * from tblInsa where ibsadate between '2008-01-12' and '2010-12-31' and city in ('서울', '경기', '인천');
